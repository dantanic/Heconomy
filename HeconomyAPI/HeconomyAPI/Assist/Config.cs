...
namespace HeconomyAPI.Assist
{
    public class Config
    {
        ...
        public Config(HeconomyAPI plugin)
        {
            ...
            if (!File.Exists(path)) File.WriteAllText(path, GetResourceString());
            string[] data = File.ReadAllLines(path);
            foreach (string item in data) LoadResource(item);
        ...
        private void LoadResource(string data)
        {
            foreach (string token in data.Split(new[] { "\r\n", "\n", Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                if ((!trim.Contains("=")) && (trim.StartsWith("#"))) continue;
        ...
        {
            if (!Contents.ContainsKey(property)) return null;
